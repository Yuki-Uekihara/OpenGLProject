...
OutputMatrix PanToMatrix(float pan);
...
public:
  ...
  virtual float SetPitch(float) override { return 1; }
  virtual float SetPan(float) override { return 1; }
  ...
  virtual float GetPan() const override { return 1; }
  virtual bool IsNull() const override { return true; }
  virtual bool DoesLoop() const override { return false; }
};
...
public:
  ...
  virtual float SetPan(float pan) override {
    if (sourceVoice) {
      const OutputMatrix om = PanToMatrix(pan);
      XAUDIO2_VOICE_DETAILS voiceDetails;
      sourceVoice->GetVoiceDetails(&voiceDetails);
      sourceVoice->SetOutputMatrix(nullptr, voiceDetails.InputChannels, om.inputChannels, om.m);
      this->pan = pan;
    }
    return pan;
  }
  ...
  virtual float GetPan() const override { return pan; }
  virtual bool IsNull() const override { return false; }
  virtual bool DoesLoop() const override { return loop; }
...
public:
  ...
  */
  bool Initialize() {
    if (xaudio) {
      LOG("WARNING: EasyAudio::Engineは初期化済みです.");
      return true;
    }
    ...
    if (FAILED(MFCreateAttributes(attributes.GetAddressOf(), 1))) {
      LOG("ERROR: Media Foundationの属性オブジェクトの作成に失敗.");
      return false;
    }
    if (FAILED(attributes->SetUINT32(MF_LOW_LATENCY, true))) {
      LOG("ERROR: Media Foundationのレイテンシの設定に失敗.");
      return false;
    }
    xaudio.Swap(std::move(tmpAudio));
    masteringVoice->GetChannelMask(&channelMask);
    masteringVoice->GetVoiceDetails(&masterVoiceDetails);
  ...
  * @code
  * EasyAudio::Engine::Instance().Prepare("ファイルパス")->Play()
  * @endcode
  *
  ...
  * EasyAudio::SoundPtr se = EasyAudio::Engine::Instance().Prepare("ファイルパス");
  * ～～～
  ...
  * パンの値から出力マトリクスを作成する
  *
  * @param pan 左右のスピーカーへの出力比(-1=左側のみ 0=中央 +1=右側のみ)
  *
  * @return パンを反映した出力マトリクス
  */
  OutputMatrix MakeOutputMatrix(float pan)
  {
    const float left = std::min(std::max(0.5f - pan / 2, 0.0f), 1.0f);
    const float right = std::min(std::max(0.5f + pan / 2, 0.0f), 1.0f);
    OutputMatrix outputMatrix = { {0} };
    switch (channelMask) {
    case SPEAKER_MONO:
      outputMatrix.m[0] = 1.0;
      break;
    case SPEAKER_STEREO:
    case SPEAKER_2POINT1:
    case SPEAKER_SURROUND:
      outputMatrix.m[0] = left;
      outputMatrix.m[1] = right;
      break;
    case SPEAKER_QUAD:
      outputMatrix.m[0] = outputMatrix.m[2] = left;
      outputMatrix.m[1] = outputMatrix.m[3] = right;
      break;
    case SPEAKER_4POINT1:
      outputMatrix.m[0] = outputMatrix.m[3] = left;
      outputMatrix.m[1] = outputMatrix.m[4] = right;
      break;
    case SPEAKER_5POINT1:
    case SPEAKER_7POINT1:
    case SPEAKER_5POINT1_SURROUND:
      outputMatrix.m[0] = outputMatrix.m[4] = left;
      outputMatrix.m[1] = outputMatrix.m[5] = right;
      break;
    case SPEAKER_7POINT1_SURROUND:
      outputMatrix.m[0] = outputMatrix.m[4] = outputMatrix.m[6] = left;
      outputMatrix.m[1] = outputMatrix.m[5] = outputMatrix.m[7] = right;
      break;
    }
    outputMatrix.inputChannels = masterVoiceDetails.InputChannels;
    return outputMatrix;
  }
...
void Resume(int playerId)
{
  ...
  SoundPtr old = engine->FindSound(playerId);
  if (old) {
    old->Play(old->DoesLoop() ? Flag_Loop : Flag_None);
  }
}
...
{
  if (!engine) {
    return OutputMatrix{ { 1, 0 }, 0 };
  }
  return engine->MakeOutputMatrix(pan);
}
